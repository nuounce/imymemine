...
/**
 * 바닥에 칠한 위치 표시 도색 — "여기 서라"는 작업장 표시다.
 *
 * 셔터 도색(`C_HAZARD`)보다 밝다. 셔터는 "지나가지 마라"라서 배경에 묻혀도
 * 되지만, 발판 표시는 **밟으라고 있는 것**이라 강철판(`C_METAL`) 위에서
 * 한눈에 읽혀야 한다. 그래도 CORE 금색(`C_LOOT`)보다는 확실히 탁하게 둬서
 * "주목물"이 아니라 "설비 표시"로 남는다.
 */
export const C_FLOOR_MARK = '#8f7a2e';
