...
export interface Pose {
   ...
   * 체격(간수는 1 보다 크다)과 **몸을 튼 정도**(측면을 보면 어깨가 겹쳐 좁아진다).
...
export interface FigureOpts {
  ...
  /**
   * 팔다리 굵기 배율(기본 1). 간수처럼 **체격 자체가 다른** 인물에만 쓴다.
...
/**
 * 정지 상태의 간수. 플레이어와 **체형과 태도가 모두** 달라야 한다:
 ...
 *
 * 명도 위계를 **몸 색에서 파생**시키는 이유: 잔상 4색·간수 붉은색·시체 회색이 전부
...
function paintTopFigure(
  ...
  /** 머리는 몸 색보다 **더 밝다.** 흰 몸(I)은 이미 상한이라 그대로지만, 붉은 간수처럼
