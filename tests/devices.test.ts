 ...
 *
 * 재기준선 (보스 4종 × 위 chaseTimer 수정 병합): `hashState` 가 `SimState.counted` 와
 * 경비의 계수 필드(countTargetId·countTimer·countCd)를 새로 섞는 변경과 위 chaseTimer
 * 수정이 합쳐진 시점의 값이다. 이 레벨의 경비는 유형 미지정 = SENTRY 하나뿐이라 보스
 * 경로(sharesScent·ghost 배율·countTicks)를 한 줄도 타지 않는다 — 세 스위치가 전부
 * 0/false 이므로 `guardSpecOf` 는 기존과 동일한 얼어붙은 객체를 그대로 돌려준다.
 * 값은 병합 후 실제 실행으로 재채록했다.
 */
const GOLDEN_SCRIPTED = [
  1199422813, 2258050745, 748873807, 2720470851, 1141199342, 568527540, 1, 2,
];
const GOLDEN_RANDOM = [
  3220910134, 3722246584, 3728251834, 2922371391, 3582653580, 3943175229, 0, 0,
];
