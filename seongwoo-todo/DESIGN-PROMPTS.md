...
## 구독(ChatGPT Plus/Pro)으로 생성하는 법 — 권장 기본 경로

API 과금 없이 **ChatGPT 화면에 §0 공통 스타일 블록 + 에셋 프롬프트를 그대로 붙여
넣으면** 구독 요금 안에서 생성된다(장당 과금 없음, 대신 생성 횟수 제한 있음).
반려는 같은 대화에서 사유를 말하며 재생성 — 맥락이 유지돼 반복 수정이 API보다 편하다.

- **투명 배경 주의**: UI 생성물은 투명이 안 깔려 나오는 경우가 있다. 파일을 열어
  확인하고, 안 되면 배경을 단색 마젠타(#ff00ff)로 받아 후처리로 제거한다(제거·슬라이스
  스크립트는 Codex에 시킨다 — Codex는 구독 로그인으로도 쓰지만 이미지는 못 그린다).
- **역할 분담**: A그룹 5개 = ChatGPT UI 수동 생성 → 사람 검수 → Codex 후처리.
  B그룹처럼 물량이 커지면 아래 API 일괄 방식을 고려한다.

## Codex에 이미지 생성을 시키는 법 (API 일괄 — 물량 클 때)
